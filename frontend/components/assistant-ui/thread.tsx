        ...
        >
          ...
          <ThreadPrimitive.ViewportFooter
            className={cn(
              ...
              !isEmpty && "sticky bottom-0 mt-auto bg-background/95 backdrop-blur-sm border-t border-border",
            )}
          >
...
const ThreadWelcome: FC = () => {
  return (
    <div className="aui-thread-welcome-root mb-8 flex flex-col items-center px-4 text-center">
      <div className="fade-in slide-in-from-bottom-2 animate-in fill-mode-both duration-300 w-full max-w-md rounded-lg border border-border border-t-4 border-t-primary bg-card px-8 py-10 shadow-sm">
...
// Seeded from the BIP Lublin sources (services / president / structure).
const SUGGESTIONS = [
  "Jak załatwić sprawę w E-Urzędzie?",
  "Kto jest prezydentem Lublina?",
  "Czym zajmują się zastępcy prezydenta?",
  "Jak wygląda struktura organizacyjna urzędu?",
];
      ...
      {SUGGESTIONS.map((prompt) => (
        ...
        >
          <ThreadPrimitive.Suggestion prompt={prompt} send asChild>
            <Button
              variant="ghost"
              className="aui-thread-welcome-suggestion text-foreground/80 hover:text-primary border-border hover:border-primary/50 h-auto gap-1.5 rounded-full border bg-card px-3.5 py-1.5 text-sm font-normal whitespace-nowrap transition-all hover:bg-accent"
            >
              {prompt}
            </Button>
  ...
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <div
        ...
        className="flex w-full flex-col gap-2 rounded-2xl border border-border bg-card p-(--composer-padding) shadow-sm transition-[border-color,box-shadow] focus-within:border-primary/50 focus-within:shadow-[0_0_0_3px_oklch(0.55_0.225_26_/_0.12)]"
      >
        <ComposerAttachments />
  ...
  return (
    ...
    >
      ...
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0">
        <div className="aui-user-message-content peer rounded-xl bg-primary px-4 py-2 text-primary-foreground shadow-sm wrap-break-word empty:hidden">
